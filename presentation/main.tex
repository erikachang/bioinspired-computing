...
%        \end{frame}
        ...
        \begin{frame}{Immunology Basics}
            \only<1>{\textbf{But how do they match each other? Is it love?}}
...
%            In pattern recognition applications, the encoding of antibodies can be a string containing the pattern recognised by those antibodies, while antigens contain the patterns that we want to be recognised.
        ...
        \begin{frame}{Affinity}
            \only<1>{So, if there's at least one thing in common between them, they match!}
            \only<2->{
            \textbf{Not necessarily!}
        
            \begin{itemize}
                \item Not any match is a (good) match!
                \item 20\% match may be as good as nothing.
                \item How do we define a good match?
            \end{itemize}
            
            \only<3>{Affinity \textbf{Threshold}!}
            
            \vfill
            \hfill\includegraphics[width=100px]{fig/cat}
            }
        \end{frame}
        ...
        \begin{frame}{Monitoring Phase}
            ...
            \begin{algorithm}[H]
                ...
                \caption{Pseudo-algorithm for the monitoring phase.}
                \end{algorithm}}
                \centering
        \end{frame}
