	...
	\introframe{Immunology Basics}{Straight from the Natural World}
        ...
        \begin{frame}{Immunology Basics}
        	...
        	\begin{itemize}
                \item B-Cells
%                \begin{itemize}
%                	\item Plasmablast (short-lived)
%                    \item Plasma Cell (long-lived)
%                    \item Memory B-Cell
%                \end{itemize}
            	\item T-Cells
%                \begin{itemize}
%                	\item T-Helper Cells
%                    \item T-Cytotoxic Cells
%                    \item T-Suppressor Cells
%                \end{itemize}
         	\end{itemize}
    	\end{frame}
...
%            In pattern recognition applications, the encoding of antibodies can be a string containing the pattern recognised by those antibodies, while antigens contain the patterns that we want to be recognised.
    ...
    \begin{frame}{Immune Networks}
        \begin{itemize}
            ...
            \item Cells stimulate and suppress one another, no need of external stimulation;
            \item Network is dynamic and is stimulated by the introduction of antigens.
        \end{itemize}
        
        \todo[inline,author=SC]{Fill this section in.}
    \end{frame}
    ...
    \begin{frame}{Immune Networks}
        ...
        \scalebox{0.75}{
        \begin{algorithm}[H]
            \KwIn{$G$, a set of antigen pairs $\langle g, c_g \rangle$}
            \KwIn{$B$, a set of antibody pairs $\langle b, c_b \rangle$}
            \KwOut{$I$, the adjusted immune network}
            
            \ForAll{$(g, c_g) \in G$} {
                \ForAll{$(b, c_b) \in B$} {
                    $a \gets $ \textit{Affinity}$(g,b)$\\
                    $\vec{y} \gets $ \textit{Affinities}$(b,B)$
                    
                    \Repeat{\textit{convergence}} {
